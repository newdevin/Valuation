...
namespace Valuation.Service
{
    public class PriceAlertSerice : IPriceAlertService
    {
        ...
        private async Task SendNotification(IEnumerable<(TargetPrice, decimal)> targetReachedPrices)
        {
            StringBuilder message = new StringBuilder();
            var subject = "Target price reached";
            foreach (var target in targetReachedPrices)
            {
                message.AppendLine($"{target.Item1.TargetType}: {target.Item1.Listing.Symbol} has met the target price of {target.Item1.Target}. New price is {target.Item2}");
            }
